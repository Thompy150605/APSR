...
% Iteration 3 - Changing The Stringer and Sparcap Configuration
...
h_st = 30;            % Height (mm)
w_st_flange = 15;     % Flange Width (mm) ~ 0.5 * h
t_st = 5.2;           % Stringer Thickness (mm)
...
t_f_cap = 2.5;          % Thickness effective (mm)
A_sparf = w_f_cap * t_f_cap; % Area ~520 mm^2

% Rear Spar Cap
w_r_cap = 64;         % Width effective (mm)
t_r_cap = 2.5;          % Thickness effective (mm)
...
if i < length(locations)
    % If there is a next point, this section ends there.
    y_end_current = locations(i+1);
else
    % If this is the last point, section ends at Wing Tip.
    y_end_current = 2.04; 
end

L_section = y_end_current - y_location;
Section_Weight = M_linear*L_section;

fprintf('Span Start (y)  : %.4f m\n', y_location);
fprintf('Span End (y)    : %.4f m\n', y_end_current);
fprintf('Section Length  : %.4f m\n', L_section);
fprintf('Linear Mass     : %.2f kg/m\n', M_linear);
fprintf('Section Weight  : %.2f kg\n', Section_Weight);
fprintf('=========================================\n');
